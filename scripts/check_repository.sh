...
required=(
  ...
  Sources/UI/SigningKeysView.swift
  Sources/UI/SignInstalledWebAppView.swift
  Sources/Signing/LocalSigningKeyStore.swift
  Sources/Signing/OnDeviceWebAppSigner.swift
  ...
  TestPackages/Hello3E-v1.4.0-Schema2-Signed.3eweb
...
assert 'NSFaceIDUsageDescription' in p
...
# M05 schema 2 package: signed time and key validity are part of signed checksums.
schema2=Path('TestPackages/Hello3E-v1.4.0-Schema2-Signed.3eweb')
with ZipFile(schema2) as z:
    manifest=json.loads(z.read('manifest.json'))
    signature=json.loads(z.read('signature.json'))
    checksums_data=z.read('checksums.json')
    checksums=json.loads(checksums_data)
    public_key.verify(base64.b64decode(signature['signature']), checksums_data)
    assert checksums['schemaVersion']==2
    assert checksums['appID']==manifest['id']
    assert checksums['version']==manifest['version']=='1.4.0'
    assert 'signedAt' in checksums
    assert 'keyValidUntil' in checksums
    for item in checksums['files']:
        assert hashlib.sha256(z.read(item['path'])).hexdigest()==item['sha256']
...
assert 'LocalSigningKeyStore' in Path('Sources/Signing/LocalSigningKeyStore.swift').read_text()
assert 'schemaVersion: 2' in Path('Sources/Signing/OnDeviceWebAppSigner.swift').read_text()
print('Info.plist, M04 trust, M05 on-device signing, catalog and samples OK')
PY
